...
- (void)reloadData {
    ...
    _headerHeight = [_fDataSource heightForHeader:self];
    _cellWidth = [_fDataSource widthForColumn:self];
    _cellHeight = [_fDataSource heightForSection:self];
    self.contentSize = CGSizeMake((_numberColumn+1)*_cellWidth, _headerHeight+_numberSection*_cellHeight);
...
- (void)loadseenItems {
    if (!_topLeftView.superview) {
        _topLeftView.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, _cellWidth, _headerHeight);
        [self.superview addSubview:_topLeftView];
    }
    for (NSInteger section=0; section<_numberSection; section++) {
        if (section*_cellHeight>self.contentOffset.y+self.frame.size.height
            || (section+1)*_cellHeight<self.contentOffset.y) {
            continue;
        }
        for (NSInteger column=0; column<_numberColumn; column++) {
            if (column*_cellWidth>self.contentOffset.x+self.frame.size.width
                || (column+1)*_cellWidth<self.contentOffset.x) {
                continue;
            }
            ...
            CGRect rect = CGRectMake((column+1)*_cellWidth, _headerHeight+section*_cellHeight, _cellWidth, _cellHeight);
    ...
    for (NSInteger section=0; section<_numberSection; section++) {
        ...
        CGRect rect = CGRectMake(self.contentOffset.x+self.contentInset.left, _headerHeight+section*_cellHeight, _cellWidth, _cellHeight);
        if ([self isOnScreenRect:rect]) {
    ...
    for (NSInteger column=0; column<_numberColumn; column++) {
        ...
        CGRect rect = CGRectMake((column+1)*_cellWidth, self.contentOffset.y+self.contentInset.top, _cellWidth, _headerHeight);
        if ([self isOnScreenRect:rect]) {
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(context, BlackLeverColor2.CGColor);
    CGPoint aPoints[5];
