...
@interface ZTTagsView () <UICollectionViewDelegate, UICollectionViewDataSource>
...
{
    ZTTagCollectionViewCell *cell = (ZTTagCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
+ (CGFloat)getHeightWithTags:(NSArray *)tags layout:(ZTTagCollectionViewFlowLayout *)layout tagAttribute:(ZTTagAttribute *)tagAttribute width:(CGFloat)width
{
    ...
    if (!layout) {
        layout = [[ZTTagCollectionViewFlowLayout alloc] init];
    }
    
    if (tagAttribute.titleSize <= 0) {
        tagAttribute = [[ZTTagAttribute alloc] init];
    }
