//
//  JTCollectionDetailExpressionTableViewCell.m
//  JTCarCustomers
//
...
@implementation JTCollectionDetailExpressionTableViewCell

- (FLAnimatedImageView *)contentImage
{
    if (!_contentImage) {
        _contentImage = [[FLAnimatedImageView alloc] init];
...
- (void)setModel:(JTCollectionModel *)model
{
    ...
    NSString *urlStr = [model.contentDic objectForKey:@"image"] ? [model.contentDic objectForKey:@"image"] : @"";
    BOOL isGif = [urlStr hasSuffix:@"gif"];
    NSURL *url;
    if (isGif) {
    ...
    {
        url = [NSURL URLWithString:[urlStr avatarHandleWithSize:CGSizeMake(size.width*2, size.height*2)]];
    }
    ...
    [self.contentImage sd_setImageWithURL:url];
}
