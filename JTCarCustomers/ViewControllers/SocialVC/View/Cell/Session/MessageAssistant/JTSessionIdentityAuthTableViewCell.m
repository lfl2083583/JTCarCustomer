//
//  JTSessionIdentityAuthTableViewCell.m
//  JTCarCustomers
//
...
@implementation JTSessionIdentityAuthTableViewCell
...
- (void)layoutSubviews
{
    ...
    if (self.model) {
        ...
        if ([attachment isKindOfClass:[JTIdentityAuthAttachment class]]) {
            self.titleLB.text = ([(JTIdentityAuthAttachment *)attachment status] == 1) ? @"实名认证成功" : @"实名认证失败";
            self.titleLB.backgroundColor = ([(JTIdentityAuthAttachment *)attachment status] == 1) ? BlueLeverColor1 : RedLeverColor1;
            self.contentLB.text = [(JTIdentityAuthAttachment *)attachment content];
        }
