...
@interface ITDatePicker () <UIPickerViewDelegate, UIPickerViewDataSource>
...
- (void)initDatePickerView {
    ...
    NSString *defaultDate = [NSString stringWithFormat:@"%ld.%02ld.%02ld", self.thisYear, self.thisMonth, self.thisDay];
    self.defaultDate = defaultDate;
    self.minimumDate = @"1970.01.01";
    self.maximumDate = defaultDate;
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView
    ...
    if (component == 0) {
        ...
        if (self.isShowToday &&
            ...
            self.defaultYear == self.thisYear &&
            self.defaultMonth == self.thisMonth &&
            self.mode == ITDatePickerModeyyyyMMdd &&
            self.defaultDay == self.thisDay) {
            text = @"至今";
        }
        return text;
...
- (void)setShowOutsideDate:(BOOL)showOutsideDate {
    ...
    if (showOutsideDate) {
        self.limitYear = self.thisYear;
        self.lowerLimitYear = 1970;
...
- (UIBarButtonItem *)cancelButton {
    if (!_cancelButton) {
        ...
        [_cancelButton setTitleTextAttributes:@{NSFontAttributeName: Font(16), NSForegroundColorAttributeName: BlueLeverColor1} forState:UIControlStateNormal];
    }
...
- (UIBarButtonItem *)confirmButton {
    if (!_confirmButton) {
        ...
        [_confirmButton setTitleTextAttributes:@{NSFontAttributeName: Font(16), NSForegroundColorAttributeName: BlueLeverColor1} forState:UIControlStateNormal];
    }
...
- (UIToolbar *)toolBar {
    if (!_toolBar) {
        ...
        _toolBar.backgroundColor = BlackLeverColor1;
